...
void draw_outline(int dx, int dy, GRect bounds, GContext *ctx, OutlinedTextData *data) {
    graphics_draw_text(ctx, data->text, data->font, 
                       (GRect){.origin={bounds.origin.x+dx+data->offset_x,bounds.origin.y+dy},.size=bounds.size},
...
void outlined_text_layer_set_offset_x(OutlinedTextLayer * layer, uint8_t offset_x){
    outlined_text_layer_get_outline_data((OutlinedTextLayer*)layer)->offset_x = offset_x;
}
